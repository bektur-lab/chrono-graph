        ...
        {state.status === "ok" && (
          <>
            <span style={{
              ...
              border: `1px solid ${state.source === "live" ? "#10b981" : "#f59e0b"}`,
            }}>
              {state.source === "live" ? "Wikidata + AI" : "mock data"}
            </span>
            <button
              onClick={() => setRefresh((r) => !r)}
              style={{ marginLeft: "auto", padding: "4px 12px", fontSize: "0.8rem", background: "#1f2937", color: "#9ca3af", border: "1px solid #374151", borderRadius: 6, cursor: "pointer" }}
            >
              Refresh
            </button>
          </>
      ...
      <div style={{ flex: 1, minHeight: 0, position: "relative" }}>
        {state.status === "loading" && (
          <div style={overlayStyle}>
            ...
            <span style={{ fontSize: "0.75rem", color: "#6b7280" }}>Wikidata → Claude AI → graph (first load ~15s)</span>
          </div>
        )}
        {state.status === "error" && (
